...
OBS_DETECT::OBS_DETECT() : rclcpp::Node("obs_detect_node")
{
    ...
    this->declare_parameter("spline_file_name", "/sim_ws/src/f1tenth_gym_ros/racelines/t5.csv");
    ...
    this->declare_parameter("drive_topic", "/ego_racecar/drive");
...
void OBS_DETECT::check_to_activate_obs_avoid(std::vector<signed char> &occugrid_flat)
{
    ...
    if (got_pose_flag == true)
    {
        ...
        int max_spline_idx = static_cast<int>(spline_points.size());
        int increment = 10;
        int iterations = 0;
        bool run_check = true;
        ...
        for (size_t i = 0; i < grid_interp_points.size(); i++)
        {
            if (grid_interp_points[i][1] >= 0 && grid_interp_points[i][0] >= 0)
            {
                size_t index = ((grid_interp_points[i][1]) * occu_grid_x_size) + (grid_interp_points[i][0]);
...
std::vector<std::vector<int>> OBS_DETECT::draw_connecting_line(int origin_point[2], int goal_point[2])
{
    ...
    if (path_line_padding == true)
    {
        int x_val;
        int y_val;
        size_t size_val = segment_interp_points.size();
...
std::vector<std::vector<int>> OBS_DETECT::bresenhams_line_algorithm(int goal_point[2], int origin_point[2])
{
    ...
    catch (...)
    {
        ...
        return output; // Return empty vector on failure
    }
}
...
int OBS_DETECT::find_obs_detect_goal_idx(float l_dist, std::vector<std::vector<float>> spline_points, int car_idx)
{
    ...
    for (size_t i = 0; i < spline_points.size(); i++)
    {
        if (current_idx >= max_spline_idx)
        {
            current_idx = 0;
        }
        if (next_idx >= max_spline_idx)
        {
            next_idx = 0;
        }
        ...
        {
            goal_point_idx = static_cast<int>(i);
            break;
        }
        current_idx += 1;
        next_idx += 1;
    ...
    if (goal_point_idx >= max_spline_idx)
    {
        goal_point_idx -= max_spline_idx;
    }
